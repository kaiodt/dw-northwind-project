    ...
    USE [DW_GOLD];
    EXEC('
        create view "dbo"."fact_orders__dbt_tmp_vw" as 
...
WITH order_details AS (
  ...
  FROM
    "DW_SILVER"."dbo"."SLV_ORDERS__order_details"
...
fact_base AS (
  ...
  FROM
    ...
    LEFT JOIN "DW_GOLD"."dbo"."dim_orders" d_orders
...
EXEC('
            SELECT * INTO "DW_GOLD"."dbo"."fact_orders" FROM "DW_GOLD"."dbo"."fact_orders__dbt_tmp_vw" 
    OPTION (LABEL = ''dbt-sqlserver'');

        ')

    
    EXEC('DROP VIEW IF EXISTS dbo.fact_orders__dbt_tmp_vw')
    ...
    if EXISTS (
        ...
        WHERE name = 'dbo_fact_orders_cci'
        AND object_id=object_id('dbo_fact_orders')
    )
    DROP index "dbo"."fact_orders".dbo_fact_orders_cci
    CREATE CLUSTERED COLUMNSTORE INDEX dbo_fact_orders_cci
    ON "dbo"."fact_orders"
