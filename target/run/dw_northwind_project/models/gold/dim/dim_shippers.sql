    ...
    USE [DW_GOLD];
    EXEC('
        create view "dbo"."dim_shippers__dbt_tmp_vw" as 
...
EXEC('
            SELECT * INTO "DW_GOLD"."dbo"."dim_shippers" FROM "DW_GOLD"."dbo"."dim_shippers__dbt_tmp_vw" 
    OPTION (LABEL = ''dbt-sqlserver'');

        ')

    
    EXEC('DROP VIEW IF EXISTS dbo.dim_shippers__dbt_tmp_vw')
    ...
    if EXISTS (
        ...
        WHERE name = 'dbo_dim_shippers_cci'
        AND object_id=object_id('dbo_dim_shippers')
    )
    DROP index "dbo"."dim_shippers".dbo_dim_shippers_cci
    CREATE CLUSTERED COLUMNSTORE INDEX dbo_dim_shippers_cci
    ON "dbo"."dim_shippers"
