    ...
    USE [DW_GOLD];
    EXEC('
        create view "dbo"."dim_customers__dbt_tmp_vw" as 
...
EXEC('
            SELECT * INTO "DW_GOLD"."dbo"."dim_customers" FROM "DW_GOLD"."dbo"."dim_customers__dbt_tmp_vw" 
    OPTION (LABEL = ''dbt-sqlserver'');

        ')

    
    EXEC('DROP VIEW IF EXISTS dbo.dim_customers__dbt_tmp_vw')
    ...
    if EXISTS (
        ...
        WHERE name = 'dbo_dim_customers_cci'
        AND object_id=object_id('dbo_dim_customers')
    )
    DROP index "dbo"."dim_customers".dbo_dim_customers_cci
    CREATE CLUSTERED COLUMNSTORE INDEX dbo_dim_customers_cci
